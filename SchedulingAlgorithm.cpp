...
SchedulingAlgorithm::SchedulingAlgorithm(DataProcess &DP, AdjMatrix &AM) {
    ...
    for(int i=0;i<carlist.size();i++){
        ...
        result =crossid_to_roadid(carlist[i].from,carlist[i].to,DP, AM,cross);
...
std::string SchedulingAlgorithm::onecar(DataProcess &DP, AdjMatrix &AM) {
    ...
    for(int i =0; i<carlist.size(); i++){
        ...
        car=crossid_to_roadid(carlist[i].from, carlist[i].to, DP, AM, cross);  //返回每一条线路
        cross.clear();
...
std::string SchedulingAlgorithm::dynamic_plan(DataProcess &DP, AdjMatrix &AM) {
    ...
    std::vector<std::vector<int>> from_to_cross; //一次时间片所有车的起始点路径
    int real_time=1;
    int random=0;
    /*********************************升序********************************************/
    ...
    from_to_cross=car_to_result(now_car, DP, AM, real_time, result); //结果存储到resul
    from_to_cross.clear();
    real_time=0;
    now_car.clear();
    road.clear();
    while(Lower_triangle.size()||Upper_triangle.size()){
        ...
        std::random_shuffle(now_car.begin(),now_car.end());  //打乱顺序
        from_to_cross=car_to_result(now_car, DP, AM, real_time, Middle_result); //结果存储到result 并返回 每个car的 cross节点序列

        Departure_schedule(DP,AM,Middle_result, real_time,from_to_cross);          //动态发车
        from_to_cross.clear();  //清除每个批次内的car 的from to 的cross 序列

        result.insert(result.end(), Middle_result.begin(), Middle_result.end());
        Middle_result.clear();

        AM.updata_Crowding_rate();     //更新每条路上的拥挤度
...
void SchedulingAlgorithm::Update_path(std::vector<std::vector<int>> &result, DataProcess &DP, AdjMatrix &AM) {
    ...
    for(int i =0; i<result.size(); i++){
        ...
        if(length>14){
            ...
            road = crossid_to_roadid(from, to, DP, AM, cross);
            cross.clear();
            result[i].clear();
...
void SchedulingAlgorithm::Departure_schedule(DataProcess &DP, AdjMatrix &AM, std::vector<std::vector<int>> &result, int &real_time,
                                             std::vector<std::vector<int>> from_to_cross) {
    std::vector<int> road;
    int Waiting_state=0;
    std::map<int,Road> roadList = DP.get_roadList();
    std::vector<Car> carList =DP.get_carList();
    float max_time, time;
    AM.cleanup_map_cars();  //先清除道路上的车辆数
    for(int i=0; i<result.size(); i++){
        if(i%2 == 0){  //每10次进行评估
            ...
            for (int j=Waiting_state; j<i; j++){
                ...
                DP.Updataroad_car(from_to_cross[j],AM);    //更新每条路上的车辆
            ...
            if(car_path_count/DP.get_road_cap() >= 0.05){ //总的容积率大于0.3，后面所有车子需要延时  DP.get_road_cap()=1912
                Waiting_state = i;
                road.clear();
                for(int m = i; m<result.size(); m++){
                    result[m][1] = real_time + (int) (max_time/270)+1;
...
std::vector<std::vector<int>> SchedulingAlgorithm::car_to_result(std::vector<Car> now_car, DataProcess &DP, AdjMatrix &AM, int flag, std::vector<std::vector<int>> &result){
    int x,y=0;
    std::vector<int> road;
    std::vector<int> cross;
    std::vector<std::vector<int>> from_to_cross(now_car.size());
    std::vector<std::vector<int>> tt(now_car.size());
    for(int i=0; i<now_car.size();i++){
        x=now_car[i].from;
        y=now_car[i].to;   //顶点
        road = crossid_to_roadid(x,y,DP,AM,cross);
        from_to_cross[i].insert(from_to_cross[i].end(), cross.begin(), cross.end());
