...
public:
    ...
    void write_map_car(int from, int to);  //获取图中的car
    ...
    void updata_Crowding_rate();                                              //更新地图拥挤度
    void Restore_map();                                                   //还原地图
    void cleanup_map_cars();
...
private:
    ...
    std::vector<std::vector<double>> GA; //二维向量存储图
    std::vector<std::vector<double>> Crowding_rate; //每条路上的拥挤率
    std::vector<std::vector<int>> cars; //每条路上的car num
    std::vector<std::vector<int>> Max_capacity; //每条道路上的最大容量

    std::vector<std::vector<double>> Backup_GA; //备份二维向量存储图
