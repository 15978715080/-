...
public:
    ...
    void Departure_schedule(DataProcess &DP, AdjMatrix &AM,std::vector<std::vector<int>> &result, int &real_time, std::vector<std::vector<int>> from_to_cross);
    ...
    std::vector<int> crossid_to_roadid(int from, int to, DataProcess &DP, AdjMatrix &AM, std::vector<int> &cross);
    std::vector<std::vector<int>> car_to_result(std::vector<Car> now_car, DataProcess &DP, AdjMatrix &AM, int flag,
