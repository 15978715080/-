...
void AdjMatrix::Creategraph(DataProcess &DP) {
    ...
    std::map<int, Road> road = DP.get_roadList();
    //初始化邻接矩阵为无限大，并且对角线为0
    std::vector<std::vector<double>>  ga(size,std::vector<double>(size,INT_MAX));
    std::vector<std::vector<int>>  ga1(size,std::vector<int>(size,0));
    std::vector<std::vector<double>>  ga2(size,std::vector<double>(size,0));
    GA.assign(ga.begin(),ga.end());      //初始化
    roadID.assign(ga1.begin(), ga1.end()); //初始化
    Crowding_rate.assign(ga2.begin(), ga2.end()); //初始化
    cars.assign(ga1.begin(), ga1.end());
    Max_capacity.assign(ga1.begin(), ga1.end());
    ...
    for(auto iter = road.begin(); iter != road.end(); iter++){
        ...
        Max_capacity[x][y] = iter->second.Max_capacity;
        roadID[x][y] = iter->second.id;

        if(iter->second.isDuplex==1){
            GA[y][x] = iter->second.length;
            Max_capacity[y][x]= iter->second.Max_capacity;
...
void AdjMatrix::write_map_car(int from, int to) {
    cars[from][to]+=1;
}

void AdjMatrix::cleanup_map_cars() {
...
void AdjMatrix::updata_Crowding_rate() {
    for(int x=0; x<size; x++){
        for(int y=0; y<size; y++){
            if(cars[x][y]>0 && Max_capacity[x][y])
            Crowding_rate[x][y] = cars[x][y]/Max_capacity[x][y];
        }
    }
}
...
//        GA[x][y] = (int)(k * (iter->second.Crowding_rate)*(iter->second.Crowding_rate) + iter->second.length);  更新每条道路的权值
        GA[x][y] = (int)(k* Crowding_rate[x][y]* Crowding_rate[x][y]) + iter->second.length;
//        GA[x][y] = iter->second.min_time;

        if(iter->second.isDuplex==1){
//            GA[y][x] = (int) (k * (iter->second.Crowding_rate)*(iter->second.Crowding_rate) + iter->second.length);
            GA[y][x] = (int)(k* Crowding_rate[y][x]* Crowding_rate[y][x]) + iter->second.length;
//            GA[y][x] = iter->second.min_time;
        }                   //k*(拥挤度+最短时间)
    }
